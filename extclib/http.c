...
static int8_t _switch_http(HTTP *http, int conn, HTTPreq *requst){
    if(!in_hashtab(http->tab, string(requst->path))){
        char buffer[PATH_SIZE];
        memcpy(buffer, requst->path, PATH_SIZE);
        int32_t index = strlen(requst->path);
        if(index == 0){
            _page404_http(conn);
            return 1;
        }
        ...
        http->funcs[index](conn, requst);
        return 0;
    }
    int32_t index = get_hashtab(http->tab, string(requst->path)).decimal;
    http->funcs[index](conn, requst);
    return 0;
}
